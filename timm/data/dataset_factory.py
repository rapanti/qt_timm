...
def create_dataset(
    ...
    """
    name = name.lower()
    input_img_mode = kwargs.pop("input_img_mode")
    ...
    elif name.startswith('tfdsicgn/'):
        ds = IterableImageDatasetICGEN(
            root,
            reader=name,
            split=split,
            is_training=is_training,
    ...
    elif name.startswith('mtlbm/'):
        name = name.split("/")[1:]
        name[-1] = name[-1].upper()
        name = "/".join(name)
        ds = StandardDataset(dataset=os.path.join(root, name),
                             image_mode=input_img_mode,
                             split=split)
    else:
